...
const schemaB = {
  ...
  steps: {
    '1_bien': [
      { path: 'bien.type', type: 'enum', values: ['appartement', 'maison', 'studio', 'immeuble', 'parking', 'local'], requirement: 'requis' },
      { path: 'bien.surface', type: 'number', unit: 'm²', min: 1, requirement: 'requis' },
      { path: 'bien.ville', type: 'string', requirement: 'facultatif' },
      { path: 'bien.codePostal', type: 'string', requirement: 'facultatif' },
      { path: 'bien.etat', type: 'enum', values: ['neuf', 'bon_etat', 'a_rafraichir', 'travaux_lourds'], requirement: 'requis' },
      { path: 'bien.anneeConstruction', type: 'number', requirement: 'facultatif' },
      { path: 'bien.copropriete', type: 'boolean', requirement: 'requis' },
      { path: 'fiscalite.holdingStructure', type: 'enum', values: ['nom_propre', 'indivision', 'sci_ir', 'sci_is', 'demembrement', 'sarl_famille', 'sas_is', 'autre'], note: "Sélecteur de cartes, défaut 'nom_propre'", requirement: 'requis (défaut nom_propre)' },
      { path: 'bien.dpe', type: 'enum', values: ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'inconnu'], note: 'Non affiché dans ce formulaire (retiré, voir conditional-logic) — synchronisé automatiquement depuis performanceActuelle.dpeActuel par le moteur (analyserArbitrage), quelle que soit la valeur saisie historiquement.', requirement: 'non saisi (dérivé)' },
    ],
    '2_historique': [
      { path: 'historique.dateAchat', type: 'date', format: 'yyyy-mm-dd', required: false, requirement: 'requis pour un verdict Conserver/Vendre (sinon dureeDetentionActuelleAns) — voir b-val-01' },
      { path: 'historique.prixAchatInitial', type: 'number', unit: '€', requirement: 'requis et > 0 — voir b-val-02' },
      { path: 'historique.fraisInitiaux', type: 'number', unit: '€', requirement: 'estimable (0 par défaut, fiabilité réduite)' },
      { path: 'historique.travauxDepuisAcquisition', type: 'number', unit: '€', requirement: 'estimable (0 par défaut, fiabilité réduite)' },
      { path: 'historique.amortissementsDejaPratiques', type: 'number', unit: '€', conditional: "visible si fiscalite.regime === 'lmnp_reel'", requirement: 'requis si LMNP réel, sinon non applicable' },
      { path: 'historique.deficitsFonciersReportables', type: 'number', unit: '€', requirement: 'estimable (0 par défaut, fiabilité réduite)' },
      { path: 'historique.dureeDetentionActuelleAns', type: 'number | undefined', unit: 'ans', note: 'Optionnel — alternative à dateAchat pour le calcul de durée de détention.', requirement: 'requis pour un verdict Conserver/Vendre (sinon dateAchat) — voir b-val-01' },
    ],
    '3_pret_en_cours': [
      { path: 'pretEnCours.pretEnCours', type: 'boolean', note: 'Toggle qui conditionne tout le reste de l’étape (voir conditional-logic).', requirement: 'requis' },
      { path: 'pretEnCours.capitalRestantDu', type: 'number', unit: '€', requirement: 'requis et > 0 si pretEnCours=true — voir b-val-03' },
      { path: 'pretEnCours.mensualiteActuelle', type: 'number', unit: '€/mois', requirement: 'requis si pretEnCours=true' },
      { path: 'pretEnCours.sourceMensualite', type: 'enum', values: ['declaree', 'recalculee'], default: 'declaree', conditional: 'visible si pretEnCours=true', note: "Choix de la mensualité utilisée pour le cash-flow du scénario Conserver (mensualité réellement déclarée, ou recalculée depuis capitalRestantDu/tauxNominal/dureeRestanteMois). La déduction fiscale des intérêts est toujours calculée sur le tableau d'amortissement recalculé, quel que soit ce choix.", requirement: 'facultatif (défaut declaree)' },
      { path: 'pretEnCours.tauxNominal', type: 'number (ratio 0-1, affiché en %)', unit: '%', requirement: 'requis si pretEnCours=true' },
      { path: 'pretEnCours.tauxAssurance', type: 'number (ratio 0-1, affiché en %)', unit: '%/an', requirement: 'estimable (0 par défaut)' },
      { path: 'pretEnCours.dureeRestanteMois', type: 'number', unit: 'mois (saisi en années, ×12)', requirement: 'requis et > 0 si pretEnCours=true — voir b-val-03' },
      { path: 'pretEnCours.dateFinPret', type: 'date | undefined', requirement: 'facultatif' },
      { path: 'pretEnCours.typePret', type: 'enum', values: ['amortissable', 'in_fine', 'differe', 'relais'], requirement: 'requis si pretEnCours=true' },
      { path: 'pretEnCours.remboursementAnticipePossible', type: 'boolean', requirement: 'requis si pretEnCours=true' },
      { path: 'pretEnCours.indemniteRemboursementAnticipe', type: 'number | undefined', unit: '€', conditional: 'visible si remboursementAnticipePossible === true', requirement: 'facultatif (calculé par défaut sinon, plafond légal)' },
      { path: 'pretEnCours.garantie', type: 'enum', values: ['hypotheque', 'caution', 'ppd', 'autre'], default: 'hypotheque', requirement: 'facultatif (défaut hypotheque)' },
      { path: 'pretEnCours.renegociationEnvisagee', type: 'boolean | undefined', requirement: 'facultatif' },
    ],
    '4_performance_actuelle': [
      { path: 'performanceActuelle.loyerActuelMensuel', type: 'number', unit: '€/mois', syncs: 'location.loyerMensuelHC', requirement: 'requis' },
      { path: 'performanceActuelle.tauxVacanceReel', type: 'number (ratio 0-1, affiché en %)', unit: '%', requirement: 'estimable (0 par défaut, fiabilité réduite)' },
      { path: 'performanceActuelle.impayesAnnuelsReels', type: 'number | undefined', unit: '€/an', requirement: 'facultatif' },
      { path: 'performanceActuelle.taxeFonciereReelle', type: 'number', unit: '€/an', syncs: 'charges.taxeFonciere', requirement: 'requis' },
      { path: 'performanceActuelle.chargesCoproReellesAnnuelles', type: 'number', unit: '€/an', conditional: 'visible si bien.copropriete === true', syncs: 'charges.chargesCoproAnnuelles', requirement: 'requis si copropriete=true' },
      { path: 'performanceActuelle.assurancePnoReelleAnnuelle', type: 'number | undefined', unit: '€/an', requirement: 'estimable (0 par défaut, fiabilité réduite)' },
      { path: 'performanceActuelle.fraisGestionReelsAnnuels', type: 'number | undefined', unit: '€/an', requirement: 'estimable (0 par défaut, fiabilité réduite)' },
      { path: 'performanceActuelle.dpeActuel', type: 'enum', values: ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'inconnu'], note: 'Source de vérité unique du DPE pour le parcours B (moteur, alertes, PDF). Synchronise bien.dpe.', requirement: 'requis' },
    ],
    '5_valeur_actuelle': [
      { path: 'valeurActuelle.valeurMarcheEstimee', type: 'number', unit: '€', requirement: 'requis et > 0 — voir b-val-02' },
      { path: 'valeurActuelle.fraisVentePct', type: 'number (ratio 0-1, affiché en %)', unit: '% du prix de vente', requirement: 'estimable (valeur par défaut, fiabilité réduite)' },
      { path: 'valeurActuelle.sourceValeur', type: 'enum', values: ['estimation_perso', 'agence', 'notaire', 'observatoire'], requirement: 'requis' },
      { path: 'valeurActuelle.fiabiliteValeur', type: 'enum', values: ['haute', 'moyenne', 'faible'], requirement: 'requis' },
      { path: 'valeurActuelle.dateVenteEnvisagee', type: 'date | undefined', requirement: 'facultatif' },
      { path: 'valeurActuelle.reemploiPrevu', type: 'string | undefined', widget: 'textarea', requirement: 'facultatif' },
    ],
    '6_alternative_objectif': [
      { path: 'alternativeReemploi.typeSupport', type: 'enum', values: ['fonds_euros', 'assurance_vie', 'etf_pea', 'scpi', 'autre'], requirement: 'requis' },
      { path: 'alternativeReemploi.rendementAnnuelAttendu', type: 'number (ratio 0-1, affiché en %)', unit: '%/an', label: 'Rendement net annuel attendu (après frais et fiscalité)', requirement: 'requis' },
      { path: 'alternativeReemploi.horizonAns', type: 'number', unit: 'ans', min: 1, requirement: 'requis' },
      { path: 'alternativeReemploi.niveauRisque', type: 'enum', values: ['faible', 'modere', 'eleve'], requirement: 'requis' },
      { path: 'alternativeReemploi.fiscaliteSupport', type: 'string | undefined', requirement: 'facultatif' },
      { path: 'objectifPatrimonial.objectifPrincipal', type: 'enum', values: ['maximiser_rendement', 'reduire_risque', 'transmission', 'liquidite', 'autre'], requirement: 'requis' },
      { path: 'objectifPatrimonial.horizonSouhaiteAns', type: 'number | undefined', unit: 'ans', min: 0, requirement: 'facultatif (défaut: alternativeReemploi.horizonAns ou 10 ans)' },
      { path: 'objectifPatrimonial.commentaireLibre', type: 'string | undefined', widget: 'textarea', requirement: 'facultatif' },
    ],
    '7_validation_envoi': [
      { path: 'lead.email', type: 'string', required: true, requirement: 'requis' },
      { path: 'lead.nom', type: 'string | undefined', requirement: 'facultatif' },
      { path: 'lead.telephone', type: 'string | undefined', requirement: 'facultatif' },
      { path: 'lead.societe', type: 'string | undefined', requirement: 'facultatif' },
    ],
  },
}
...
const validationRulesB = {
  ...
  engineGuards: [
    { rule: 'historique.dateAchat vide ET dureeDetentionActuelleAns non renseignée', effect: "detailPlusValue neutralisée (N/A), dateAcquisitionConnue = false (voir b-cl-08). Le verdict est alors forcé à 'Arbitrage à approfondir' (couleur jaune) avec une alerte dédiée — voir b-val-01." },
    ...
    { rule: "performanceActuelle.dpeActuel === 'F' || 'G'", effect: "Alerte DPE (difficulté de location, gel des loyers, interdiction de location à venir) ajoutée à verdict.alertes. Si les loyers actuels sont neutralisés à 0 € dans le tableau annuel du scénario Conserver (cas DPE G concerné par l'interdiction de mise en location), une alerte dédiée décrit cette neutralisation ; sinon, une alerte signale que la perception des loyers actuels sur tout l'horizon est une hypothèse optimiste (le scénario Conserver doit être lu comme un majorant)." },
    { rule: 'valeurActuelle.valeurMarcheEstimee <= 0', effect: "Ajout à motifsBlocage ('valeur de marché actuelle non renseignée ou nulle') — voir b-val-02." },
    { rule: 'historique.prixAchatInitial <= 0', effect: "Ajout à motifsBlocage ('prix d'achat initial non renseigné ou nul') — voir b-val-02." },
    { rule: 'pretEnCours.pretEnCours === true ET (capitalRestantDu <= 0 OU dureeRestanteMois <= 0)', effect: "Ajout à motifsBlocage (CRD ou durée restante du prêt non renseignés/nuls) — voir b-val-03." },
    { rule: 'pretEnCours.pretEnCours === true ET écart entre mensualiteActuelle et la mensualité recalculée (capitalRestantDu/tauxNominal/dureeRestanteMois) > 10 %', effect: "Alerte ajoutée à verdict.alertes décrivant l'écart (montants formatés via fmtEur, pas de séparateur U+202F) et précisant quelle mensualité (déclarée ou recalculée, selon pretEnCours.sourceMensualite) alimente le cash-flow du scénario Conserver. Si l'écart dépasse 15 %, le verdict est dégradé en 'Arbitrage à approfondir' — voir b-val-04." },
  ],
  blockingErrors:
    "Le parcours B ne bloque pas la soumission du formulaire, mais le moteur (analyserArbitrage) peut désormais déclarer un arbitrage 'non exploitable en l'état' via verdict.motifsBlocage (P1#11) : " +
    "si la valeur de marché actuelle (valeurActuelle.valeurMarcheEstimee), le prix d'achat initial (historique.prixAchatInitial), ou — lorsqu'un prêt en cours est déclaré — son capital restant dû (pretEnCours.capitalRestantDu) et sa durée restante (pretEnCours.dureeRestanteMois) sont manquants ou nuls, " +
    "le verdict est forcé à 'Arbitrage à approfondir' avec couleur rouge, une alerte 'Non arbitrable en l'état : ...' listant les motifs, et des recommandations invitant à compléter ces champs. " +
    "Par ailleurs, si la date d'acquisition n'est pas connue (voir b-cl-08), le verdict est dégradé en 'Arbitrage à approfondir' (couleur jaune) même hors motifsBlocage, car la fiscalité de cession ne peut pas être calculée.",
}
...
const fieldToEngineMappingB = {
  ...
  mappings: [
    ...
    { formField: 'pretEnCours.sourceMensualite', engineUsage: ["construireScenarioConserver() — choix de la mensualité (déclarée ou recalculée) utilisée pour le cash-flow", "ecartMensualitePct (alerte + dégradation du verdict si écart important, voir b-val-04)"], pdfUsage: ["Tableau annuel — Cash-flow (montant selon le choix)", "Alerte 'Écart entre la mensualité déclarée et recalculée' si écart > 10 %"], note: "La déduction fiscale des intérêts reste toujours calculée sur le tableau d'amortissement recalculé, quel que soit ce choix." },
    ...
    { formField: 'valeurActuelle.valeurMarcheEstimee / historique.prixAchatInitial / pretEnCours.{capitalRestantDu,dureeRestanteMois}', engineUsage: ["verdict.motifsBlocage (construireVerdict) — champs structurants sans lesquels l'arbitrage chiffré est non exploitable"], pdfUsage: ["Bannière verdict (couleur rouge si motifsBlocage non vide)", "Alerte 'Non arbitrable en l'état : ...'"], note: 'P1#11 — voir b-val-01/02/03.' },
  ],
}
